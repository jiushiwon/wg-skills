...
const CurrentSubtitle: React.FC<{
  ...
  totalFrames: number;
  index: number;
}> = ({ text, relativeFrame, totalFrames, index }) => {
  const ENTER_DURATION = 18; // 入场 0.6s
  const EXIT_START_FRAME = totalFrames - 20; // 离场提前 0.66s 开始

  // 入场：从下方 80px 滑入 + 缩放 0.6→1 + 透明度 0→1
