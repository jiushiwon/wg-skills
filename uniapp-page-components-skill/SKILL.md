...
description: uniapp 常用「组件化页面」生成技能。内置 6 类高频页面组件（Tab+列表 / 聊天 / 朋友圈 / 商品详情 / 我的 / 图片卡片），以空卡片 BaseCard 为托底组件，页面入参含 BaseCard 全部入参，slot 自由填充，自动接入 uniapp-theme-skill 主题系统（CSS 变量、动态颜色尺寸、禁止写死）。触发词："组件化页面"、"常用页面"、"聊天页"、"朋友圈"、"商品详情页"、"我的页面"、"订单列表页"、"Tab 列表"、"图片卡片"、"空卡片组件"、"BaseCard"、"uniapp 页面组件"
---
...
2. 检测目标项目是否已有主题系统：查找 `src/styles/theme/` 或全局样式中是否存在 CSS 变量 `--color-primary` / `--primary-500` / `--radius-card` / `--spacing-md` 等。
   - **有主题系统**：直接使用组件模板，无需改动。
   - **无主题系统**：二选一——
     - 询问用户是否先调用 `uniapp-theme-skill` 初始化主题系统（推荐，后续统一换肤）；
     - 或按 `references/theme-integration.md` 的「无主题系统 fallback 硬编码替换表」把每个 `var(--xxx)` 替换为具体值（允许写死，SKILL.md 顶层段落可给出说明）。
