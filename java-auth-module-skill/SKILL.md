---
name: java-auth-module-skill
description: Java（Spring Boot）授权与企业组织架构模块一键叠加技能。面向已使用 springboot-init-skill 或 java-backend-skill 生成的项目，标准化落地 RBAC、角色-菜单绑定、部门/岗位/租户、数据权限、个人鉴权。触发词："Java 授权模块","Spring Boot 权限模块","RBAC 模块","角色菜单权限","企业组织架构","java-auth-module","添加权限模块","帮我加一个 Java 鉴权模块"。
---
...
- 目标：在已有 `springboot-init-skill` / `java-backend-skill` 骨架上，添加可运行的 RBAC + 组织架构模块。
...
```
1. 现有项目的 Java 包名是什么？（默认从 springboot-init-skill 推断，如 com.koala.myapp）
2. 表前缀是什么？（默认 wg）
...
企业组织架构、java-auth-module、添加权限模块、帮我加一个 Java 鉴权模块
```
