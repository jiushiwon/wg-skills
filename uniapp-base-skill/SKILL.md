...
| 弹窗组件 | 根组件 | popup 属于页面性质，依赖 base-card |
| 页面模板 | card + form | 组合使用卡片和表单组件 |

### 组件放置规则

1. **根组件** → 放在 `uniapp-base-skill/` 根目录
2. **表单组件** (input/switch/radio/select) → 放在 `uniapp-form-skill/`
3. **弹窗组件** (popup) → 放在 `uniapp-page-skill/`（页面性质）
4. **卡片组件** → 放在 `uniapp-card-skill/`
5. **页面模板** → 放在 `uniapp-page-skill/`
