...
trigger: |
  ...
  # 卡片布局（8套风格）
  做一个卡片 | 基础卡片 | 图片卡片 | 列表卡片 | 信息卡片 | 个人中心卡片
  做一个好友卡片 | 做一个设置卡片 | 做一个菜单卡片 | 做一个商品卡片 | 做一个VIP卡片
  做一个评论卡片 | 做一个通知卡片 | 做一个功能网格卡片 | 做一个网格卡片
  # 固定底部按钮
  ...
  # 输入场景（账号/密码/验证码/反馈/只读/图标/OTP/浮动标签/搜索栏变体）
  # 布局与导航
  # 登录页（7种）
...
| base-input | 输入框 | 账号/密码/手机号/验证码/多行文本/图标/OTP/浮动标签/搜索栏（搜索栏作为变体形态） |
| base-avatar | 头像 | 头像组件 |
| base-tag | 标签 | 标签角标 |
...
- ✅ 新增按钮组件 `base-button.md`：6 套风格（主色实心、主色描边、幽灵按钮、胶囊按钮、圆角方形、渐变按钮）
- ✅ 新增卡片布局 `base-card-layout.md`：6 套布局（基础卡片、图片卡片、列表卡片、信息卡片、表单卡片、个人中心卡片）
...
- ✅ 新增通用输入框组件 `base-input.md`：账号/密码/手机号/验证码/多行文本/图标/OTP/浮动标签统一入口
- ✅ 输入框案例集重构：原 `search/` 并入 `base-input/`，共 14 个场景（8 通用 + 6 搜索栏独立形态）
- ✅ 8 个通用输入：login / verify / feedback / disabled / icon-prefix / icon-suffix / otp / floating
- ✅ 6 个搜索栏独立形态：search-pill / search-card / search-bubble / search-flat / search-embed / search-mini
- ✅ 每个场景独立成 HTML + MD 文档，按形态检索复用
- ✅ 圆角变体演示：覆盖 0 / 8px / 12px / 999px 四档
