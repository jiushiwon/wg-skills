...
`remotion-skill` 是 wg-skills 仓库中面向**社媒短视频**生成的技能。它与现有 `ffmpeg-skill` 形成互补关系：

- `remotion-skill`：负责**程序化生成**视频（React → MP4）
- `ffmpeg-skill`：负责**后期处理**（压缩、转码、加封面等）
...
3. **不捆绑 FFmpeg**：渲染后处理调用系统 ffmpeg，未安装时提示调用 ffmpeg-skill 安装。
...
- `ffmpeg-skill`：后期处理依赖，提供安装检测和命令参考
