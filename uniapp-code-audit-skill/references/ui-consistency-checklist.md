...
| 检查项 | 风险等级 | 风险描述 | 判定依据 | 参考标准 | 检测命令 |
|--------|----------|----------|----------|----------|----------|
| 标题字号不统一 | P2 | 视觉层级混乱 | 同层级标题使用不同 `font-size` | `uniapp-app-generate-skill` theme-system | `grep -rnE 'font-size:\s*\d+rpx' src/` |
| 正文字号散乱 | P2 | 阅读体验不一致 | 正文出现多种字号 | `uniapp-app-generate-skill` theme-system | `grep -rnE 'font-size:\s*\d+rpx' src/pages/` |
...
| 检查项 | 风险等级 | 风险描述 | 判定依据 | 参考标准 | 检测命令 |
|--------|----------|----------|----------|----------|----------|
| margin 值散乱 | P3 | 间距系统未落地 | margin 出现非 4/8/12/16/24/32/40/48rpx 的随机值 | `uniapp-app-generate-skill` theme-system | `grep -rnE 'margin[^:]*:\s*\d+rpx' src/` |
| padding 值散乱 | P3 | 间距系统未落地 | padding 出现非 Token 的随机值 | `uniapp-app-generate-skill` theme-system | `grep -rnE 'padding[^:]*:\s*\d+rpx' src/` |
...
| 检查项 | 风险等级 | 风险描述 | 判定依据 | 参考标准 | 检测命令 |
|--------|----------|----------|----------|----------|----------|
| 圆角值不统一 | P3 | 卡片/按钮视觉不一致 | 同类组件使用不同圆角值 | `uniapp-app-generate-skill` theme-system | `grep -rnE 'border-radius:\s*\d+rpx' src/` |
...
| 页面手写原生 `<button>` | P1 | 未使用 `AppButton` 共享组件 | `src/pages/` 中出现 `<button` | `uniapp-app-generate-skill` component-standards | `grep -rn "<button" src/pages/` |
...
| 共享组件内部使用裸色值/裸尺寸 | P1 | 破坏主题系统一致性 | `src/components/` 中出现 `#hex` 或具体 `rpx` | `uniapp-app-generate-skill` component-standards | `grep -rnE '#[0-9a-fA-F]{3,6}\|font-size:\s*\d+rpx\|margin[^:]*:\s*\d+rpx' src/components/` |

## 6. 交互反馈

| 检查项 | 风险等级 | 风险描述 | 判定依据 | 参考标准 | 检测命令 |
|--------|----------|----------|----------|----------|----------|
| 按钮无点击态 | P3 | 用户无法感知点击 | `<button>` 或自定义按钮未设置 `hover-class` | `uniapp-app-generate-skill` component-standards | `grep -rn "<button" src/ \| grep -v 'hover-class'` |
...
| 混用 `px` 与 `rpx` | P2 | 跨端尺寸不一致 | 样式中同时出现 `px` 和 `rpx` | `uniapp-app-generate-skill` cross-platform | `grep -rnE '\d+px' src/ \| grep -v 'upx'` |
