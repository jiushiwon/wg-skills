...
| 项 | 值 |
|----|-----|
| 定位 | fastapi-init-demo 项目 |
| 技术栈 | Python + FastAPI + SQLAlchemy 2.0 异步 + SSE（sse-starlette）（版本见根目录 `versions.md`） |
| 数据库 | MySQL 8.0，表前缀 `wg_` |
| 中间件 | security_headers / request_log / CORS / exception（无则写"无"） |
| 默认端口 | 8080（环境变量 `APP_PORT` 可改） |
...
2. 创建 `app/models/post.py` 定义 SQLAlchemy ORM 模型（表名 `wg_posts`）
