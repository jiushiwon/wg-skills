...
async def delete_session(
    session_id: int,
    hard: bool = Query(False, description="是否硬删除（同时清除消息），默认软删除"),
    current_user: CurrentUser = Depends(get_current_user)
):
    """删除会话

    - 默认软删除（仅置 deleted_at）
    - ?hard=true 硬删除（同时级联删除 messages）
    """
    await SessionService.delete_session(session_id, current_user.id, hard_delete=hard)
    return {"deleted": True, "session_id": session_id, "hard": hard}
