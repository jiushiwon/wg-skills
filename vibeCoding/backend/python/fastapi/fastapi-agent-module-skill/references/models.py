...
# ✅ 修复 P2-6: 使用 datetime.now(UTC) 替代 deprecated utcnow()
...
class AgentSession(SQLModel, table=True):
    ...
    user_id: int = Field(description="关联用户ID", index=True)
...
class AgentMessage(SQLModel, table=True):
    ...
    id: Optional[int] = Field(default=None, primary_key=True)
    session_id: int = Field(description="会话ID", index=True)
