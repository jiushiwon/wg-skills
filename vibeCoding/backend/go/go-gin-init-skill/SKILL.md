...
- **database-design-skill**：MySQL / PostgreSQL 选型规则、表前缀 `wg`、连接参数
...
1. 检测 Go 是否安装 / 版本（需 >= 1.20）
2. 检测 GOPATH / GOROOT 是否可用
...
按 `references/skeleton.md` 的目录结构与代码模板，现场生成全部文件。维护者可用本 skill 根目录的 `scripts/generate_project.go` 作为 canonical 生成器参考，确保所有文件一次生成、编码正确（`.bat` 为 UTF-8 with BOM + CRLF）。

生成顺序：
1. 创建目录结构
2. 初始化 go.mod（`go mod init`）
3. 写入依赖与配置（`.env.example`、`.env`、`.gitignore`）
4. 写入核心模块（main.go、config.go、database.go、response.go、exceptions.go、middleware.go）
5. 写入业务模块（models → handlers → services → routes）
6. 写入启动脚本（`restart.sh` / `restart.bat`，dev/prod 双模式）
7. 写入 Docker 配置（Dockerfile + docker-compose.yml / docker-compose.pg.yml，按需启用）
8. 写入强制交付物（api-contract.md + docs/project-guide.md）
9. 写入项目说明（README.md）
...
3. 安装 air 热重载（`go install github.com/air-verse/air@latest`）
4. 编译检查（`go build -o bin/server .`）
...
📁 生成的文件（约 20 个）：
  - 核心模块：main.go, config.go, database.go, response.go, exceptions.go, middleware.go
  - API 路由：health / auth / users / sse / upload
  - 启动脚本：restart.sh, restart.bat（dev/prod 双模式）
  - 数据库：MySQL（已配置 docker-compose.yml，可选 PG / 无数据库）
...
- 表前缀：`wg`（可在 .env 中修改）
- 应用端口：`8080`
- Swagger：`/swagger/index.html`、`/docs`
- 健康检查：`GET /api/health`
...
| 文件 | 内容 |
|------|------|
| `scripts/generate_project.go` | canonical 项目生成器：从 `skeleton.md` / `startup-scripts.md` 提取模板并生成完整项目 |
| `references/skeleton.md` | 精简目录结构 + 核心文件代码模板（main/config/database/middleware/handlers/services/models） |
| `references/env-setup.md` | 环境探测流程、自动安装逻辑、常见问题排错 |
| `references/sse-guide.md` | SSE 流式框架集成方案、示例端点、客户端对接 |
| `references/db-guide.md` | 数据库选型、MySQL/PG 连接配置、Docker 启动命令 |
...
9. **`.env` 与 `.gitignore` 必须随脚手架一起生成，且 `.env` 中的配置必须被服务加载**：`config.go` 通过 `godotenv` 读取 `.env` 全部配置，禁止在代码中硬编码端口、数据库密码、JWT 密钥等运行时可变参数。

## 触发关键词清单

```
Go 脚手架、Go 一键生成、初始化 Go 项目、Go 快速开始、
go init、搭建 Go 服务、Go Web 骨架、Go 开箱即用、
Go 零基础、Go 小白、帮我搭一个 Go、新建 Go、
create go project、go starter
```

## 相关技能

- **go-auth-module-skill**：在生成的项目上叠加 RBAC + 组织架构模块（用户、角色、菜单、部门、岗位、租户、数据权限）
