...
<template>
  ...
  >
    <span class="base-checkbox__input">
      ...
      >
        <!-- 半选横线（inline SVG） -->
        <svg v-if="indeterminate" class="base-checkbox__icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
          <line x1="3" y1="8" x2="13" y2="8"/>
        </svg>
        <!-- 选中对勾（inline SVG） -->
        <svg v-else-if="isChecked" class="base-checkbox__icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
...
.base-checkbox__icon {
  width: 70%;
  height: 70%;
  color: #fff; /* 选中态：白色（在 primary 背景上）*/
  pointer-events: none;
}
