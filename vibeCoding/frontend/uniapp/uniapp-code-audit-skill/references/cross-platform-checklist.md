...
|--------|----------|----------|----------|----------|----------|
| 使用 H5 标签 | P0 | 小程序端无法识别 | 模板中出现 `div`、`span`、`p`、`h1~h6`、`img`、`section`、`article`、`main`、`ul`/`li`/`ol` | `uniapp-app-generate-skill/references/cross-platform-compatibility.md` | `grep -rnE '<div\|<span\|<p\|<h[1-6]\|<img\|<section\|<article\|<main\|<ul\|<li\|<ol' src/` |
...
> **说明**：CSS 自定义属性 `var(--xxx)` 是 uniapp 官方推荐的主题方案（详见 `uniapp-app-generate-skill/references/theme-system.md`），**不作为风险项**，审计时不得误报。

| 检查项 | 风险等级 | 风险描述 | 判定依据 | 参考标准 | 检测命令 |
|--------|----------|----------|----------|----------|----------|
| 使用 `background-image: url()` | P1 | 小程序端图片表现不一致 | 样式中出现 `background-image: url(...)` | `uniapp-app-generate-skill/references/cross-platform-compatibility.md` | `grep -rnE 'background-image:' src/` |
| 使用 `calc()` | P2 | 多端计算结果不一致 | 样式中出现 `calc(` | `uniapp-app-generate-skill/references/cross-platform-compatibility.md` | `grep -rnE 'calc\(' src/` |
| 使用 `vw`/`vh` | P2 | 小程序端适配问题 | 样式中出现 `vw`/`vh` | `uniapp-app-generate-skill/references/cross-platform-compatibility.md` | `grep -rnE '[0-9]+vw\|[0-9]+vh' src/` |
...
| 使用 `position: fixed` | P2 | 小程序端 z-index 与层级差异 | 样式中出现 `position: fixed` | `uniapp-app-generate-skill/references/cross-platform-compatibility.md` | `grep -rnE 'position:\s*fixed' src/` |

## 3. API 调用

| 检查项 | 风险等级 | 风险描述 | 判定依据 | 参考标准 | 检测命令 |
|--------|----------|----------|----------|----------|----------|
| 使用 `fetch` | P1 | 小程序端无原生 `fetch` | 代码中出现 `fetch(` | `uniapp-app-generate-skill/references/cross-platform-compatibility.md` | `grep -rnE 'fetch\(' src/` |
| 使用 `window`/`document` | P1 | 小程序端不存在 | 代码中出现 `window.` 或 `document.` | `uniapp-app-generate-skill/references/cross-platform-compatibility.md` | `grep -rnE 'window\.|document\.' src/` |
| 使用 `localStorage`/`sessionStorage` | P1 | 小程序端不可用 | 代码中出现 `localStorage` 或 `sessionStorage` | `uniapp-app-generate-skill/references/cross-platform-compatibility.md` | `grep -rnE 'localStorage\|sessionStorage' src/` |
| 使用 `alert` | P2 | 小程序端不支持 | 代码中出现 `alert(` | `uniapp-app-generate-skill/references/cross-platform-compatibility.md` | `grep -rnE 'alert\(' src/` |
...
| 样式 `var()`/`calc()` 未按端验证 | P2 | 主题变量在小程序/App 端失效 | 使用 `var(--xxx)`/`calc()` 未在三端逐一验证 | `uniapp-app-generate-skill/references/cross-platform-compatibility.md` | 检查 `var(`/`calc(` 出现处并核对各端构建产物 |
