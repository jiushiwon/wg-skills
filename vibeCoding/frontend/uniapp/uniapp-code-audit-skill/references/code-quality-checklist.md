...
> **Vue2 适配说明**：本节检查项以 Vue3 + Composition API 为基线。审计目标为 Vue2 + Options API 项目时：
> - `script setup`、`ref`/`reactive`、`watch`、`onUnmounted` 等检查项整体标注为 **"不适用"**，报告中列出"Vue2 项目，Composition API 检查项跳过"；
> - 仍保留检查项用于将来升级到 Vue3 后的对照基线（详见 `uniapp-vue2-upgrade-skill`，本地保留，未纳入版本控制）；
> - 通用内存泄漏检查（清理 `setTimeout`/`uni.$on` 等副作用）在 Vue2 中同样适用，不跳过。

| 检查项 | 风险等级 | 风险描述 | 判定依据 | 参考标准 | 检测命令 |
|--------|----------|----------|----------|----------|----------|
| 未使用 `script setup` | P3 | 代码冗余 | 新项目组件仍使用 Options API 或冗长 setup 函数 | `uniapp-app-generate-skill` 3.6 | 抽样检查 `.vue` 文件 |
| 生命周期未清理副作用 | P1 | 内存泄漏 | `watch`、`setTimeout`、`uni.$on` 等未在 `onUnmounted` / `destroyed` 中清理 | 通用代码规范（内存与生命周期） | 检查组件生命周期 |
...
| 检查项 | 风险等级 | 风险描述 | 判定依据 | 参考标准 | 检测命令 |
|--------|----------|----------|----------|----------|----------|
| 项目无任何测试 | P2 | 关键逻辑无回归保障 | `package.json` 无 `test` 脚本或 `src/` 无测试文件 | 通用工程规范 | 检查 `package.json` 与 `src/**/*.spec.ts` / `*.test.ts` |
| 核心逻辑未覆盖 | P2 | 工具/API/状态变更无验证 | `src/utils/`、`src/api/`、`src/stores/` 关键逻辑无单测 | 通用工程规范 | 检查上述目录测试文件 |
| 无覆盖率统计 | P3 | 回归风险无法量化 | 无覆盖率配置 | 通用工程规范 | 检查 `vitest.config.ts` / `jest.config.*` |

## 11. 埋点与数据采集

| 检查项 | 风险等级 | 风险描述 | 判定依据 | 参考标准 | 检测命令 |
|--------|----------|----------|----------|----------|----------|
| 关键事件未埋点 | P2 | 注册/支付/分享等转化无法分析 | 关键行为无埋点调用 | 通用产品规范 | 检查埋点封装与关键页面逻辑 |
| 埋点未统一封装 | P2 | 逻辑散落、维护困难 | 各页面直接调用第三方 SDK 而非统一封装 | 通用工程规范 | 检查 `src/utils/track.ts` 或等价封装 |
| 埋点参数含敏感信息 | P1 | 用户隐私泄露 | 埋点参数携带 Token、完整手机号等 | 通用安全规范 | 检查埋点封装上报参数 |
