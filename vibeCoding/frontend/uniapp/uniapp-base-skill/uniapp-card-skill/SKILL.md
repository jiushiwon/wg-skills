...
trigger: |
  ...
  # 信息流卡片
  做一个朋友圈卡片 | 做一个动态卡片 | 做一个社交动态卡片 | 做一个视频卡片
  做一个文章卡片 | 做一个新闻卡片 | 做一个资讯列表 | 做一个话题卡片
  做一个优惠券卡片
  # 图表卡片（canvas 2d 跨端兼容，无 echarts 等图表库）
  做一个折线图卡片 | 做一个趋势图 | 做一个柱状图卡片 | 做一个对比图
  做一个饼图卡片 | 做一个环形图 | 做一个雷达图卡片 | 做一个能力评估
  做一个进度环卡片 | 做一个任务进度 | 做一个仪表盘卡片 | 做一个健康指数
  # 按钮组件（6套风格）
