...
3. **生成优于搜索**：优先使用 `image-forge-skill` 技能生成图标。
...
| 资源 | 路径 | 用途 | 生成方式 |
|------|------|------|----------|
| 首页 tabBar 图标 | `static/tab-bar/home.png` | 底部导航 | image-forge-skill |
| 数据 tabBar 图标 | `static/tab-bar/data.png` | 底部导航 | image-forge-skill |
| 发现 tabBar 图标 | `static/tab-bar/discover.png` | 底部导航 | image-forge-skill |
| 我的 tabBar 图标 | `static/tab-bar/profile.png` | 底部导航 | image-forge-skill |
| 空状态图 | `static/images/empty-data.png` | 数据为空 | Pexels API |
| 头像占位 | `static/avatar-placeholder.png` | 未获取头像 | Pexels API / image-forge-skill |
