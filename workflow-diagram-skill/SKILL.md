...
description: Use when 用户想用一句话生成流程图、工作流图解、开发流程图、AI 流程图或任何步骤型图解，支持多风格输出（flat icon / 手绘草图 / dark），并提供现成模板一键出图。
---
...
4. 默认风格为 `flat`，用户可指定 `sketchy` / `dark`。
...
**Q：风格参数怎么传？**  
A：`scripts/generate_diagram.py --style flat|sketchy|dark --template vibe-coding -o output.svg`
